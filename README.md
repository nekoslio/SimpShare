...
小米社区、闲鱼、京东 / 淘宝天猫 / 拼多多商品页、百度网盘带提取码链接这类页面，
用 HttpURLConnection 拿不到数据，短链的重定向链还含 JS 跳转。规则里声明
`render: true`（或 `{ "minMs": 9000, "coverFromDom": true, "descFromDom": {…} }`）后，
分享时会起一个不挂界面的无头 WebView，交给真实内核加载：HTTP 302、JS 跳转、SPA 渲染
全部发生完（地址/标题/og 快照连续稳定，且不早于 `minMs`）后，再从渲染好的
DOM 里取标题、简介、封面和 favicon，随后销毁 WebView。`coverFromDom` 取正文首图
当封面（选图跳过头像/logo/UI 素材，懒加载先滚动扫描再取图）；`descFromDom` 按选择器
取描述文本，剥离与标题重复的前缀（剥离点延伸到下一个标点/换行，正好从"第二行/下一句"
开始），超出截断加省略号。百度网盘的 `minMs` 就是为"自动验证提取码 → 跳文件
列表页 → 标题写入文件名"这段慢流程留的余量。

商品页需要登录才能拿到完整信息，而 SimpShare 的无头 WebView 与闲鱼 App 并不共享
Cookie——应用主页新增了「网页登录」卡片：在内置 WebView 里登录闲鱼 / 淘宝 / 京东 /
拼多多网页版（Cookie 只存本应用），之后的无头捕获即为登录态，卡片升级为
商品名 + 商品图 + 描述。
闲鱼商品详情接口有登录门禁：App 内分享出的文本自带【标题】，`shareHint` 规则用它兜底标题；
浏览器侧登录态下无头捕获直接拿到商品名、商品首图与描述。未登录时自动降级二维码模式。
京东 / 淘宝天猫 / 拼多多商品页同样登录门禁：`u.jd.com`、`m.tb.cn`、
`p.pinduoduo.com` 等 App 分享短链经跳转直达商品页，无头捕获登录态渲染后的商品名
（去"【行情 报价 价格 评测】-京东"、"-tmall.com天猫"等样板后缀）与商品首图；App 分享文本
里的【标题】做兜底。
