...
内置 44 条站点规则（`rules.json` v6）。重点站点走精确提取：
...
- **京东 / 淘宝天猫 / 拼多多**：商品页全部登录门禁或纯 CSR——`render` 无头捕获登录态渲染后的
  商品名（去"【行情 报价 价格 评测】-京东"、"-tmall.com天猫"等样板后缀）与商品首图，
  `u.jd.com`、`m.tb.cn`、`p.pinduoduo.com` 等 App 分享短链经跳转直达商品页
- **闲鱼**：卡片最完整——登录态下标题即商品名（去 `_闲鱼` 后缀）、封面取商品首图，
  描述模块（图片与二维码之间）用 `descFromDom` 取商品描述：标题前缀剥离点自动延伸到
  下一个标点/换行，正好从"第二行/下一句"开始，超出截断加省略号；未登录降级二维码模式
...
后才出现，`coverFromDom` 取帖子配图当封面）、**京东 / 淘宝天猫 / 拼多多 / 闲鱼**
（商品页全部登录门禁，登录态下渲染出商品名与商品图，未登录降级二维码模式）、
**百度网盘**（带提取码的链接在页面内自动验证跳列表页后标题才是文件名，`minMs` 保证
不早收）、**123云盘** 按此处理。
